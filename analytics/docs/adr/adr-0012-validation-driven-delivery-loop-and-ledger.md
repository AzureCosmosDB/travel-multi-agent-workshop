...
|---|---|---|---|
| B1 | **Agent-execution (node) grain** re-instrumentation | **Grounded — verified live (2026-08-01)** | Wired into `travel_agents_api.py` (per-node capture) + `services/node_executions.py` (self-provisioning `NodeExecutions`). **Live-verified against `cosmos-f2tx5x7js4bwi` (keyless/Entra):** node executions written to Cosmos and read back by the engine (`data/verify_live.py`). Remaining: capture from a real agent turn (needs the full app running). |
...
| B19 | **Business-outcome linkage** (stamp a correlation key on the domain outcome) | **Grounded — verified live (2026-08-01)** | `session_id` threaded through identity injection → `create_new_trip` → `create_trip` persists `sessionId`. **Live-verified against `cosmos-f2tx5x7js4bwi`:** `create_trip(session_id=…)` persists `sessionId` and reads back correctly (`data/verify_live.py`). |
