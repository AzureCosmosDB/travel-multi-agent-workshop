...
| B19 | **Business-outcome linkage** (stamp a correlation key on the domain outcome) | Spike (grounded gap) | Today `create_trip` (`azure_cosmos_db.py:819`) persists `Trips` with `status` but **no `sessionId`/`turn_id`**, and `Trips` are partitioned `[tenantId, userId, tripId]` — so the outcome↔execution join is **coarse** (`[tenant,user]` + time, not per-session). Fix = stamp `sessionId`/`turn_id` on the outcome record (or emit `record_outcome`). Exit: a runtime-created outcome carries the correlation key → `WorkflowExecution ⋈ outcome` at session grain yields cost-per-outcome. *(No framework provides a business-outcome primitive; §10.4.)* |
| B20 | **Fabric built-in engine-LLM viability on F2** | Spike | *Verified:* F64 minimum removed Apr 2025 → AI functions run on **F2**; but F2 is 2 CU/s (small batches only). *Unverified:* the **built-in model catalog** (historically limited to `gpt-4o`/`gpt-35-turbo`/`ada-002`; "Foundry in Fabric" may broaden it). Exit: confirm on the live F2 workspace *which* models the notebook can call and whether they meet the judge/analyst quality bar. **If inadequate, the default engine-model flips to the external Azure OpenAI path** (§5.3) — the configurable design already supports this. |
