...
| B19 | **Business-outcome linkage** (stamp a correlation key on the domain outcome) | **Spike — mechanism grounded (2026-08-01); app code-wiring pending** | `analytics/spikes/b19_outcome_linkage.py`: proves that WITHOUT `sessionId` the session-grain join is impossible (coarse per-user only), and WITH one stamped key `WorkflowExecution ⋈ Trips ON sessionId` yields correct per-session cost-per-outcome (isolates a costly non-converting session). The gap is real: `create_trip` (`azure_cosmos_db.py:819`) persists `Trips` (`status`) but **no `sessionId`/`turn_id`**. Remaining: thread `session_id` through `create_new_trip` and persist it (one-field change). |
