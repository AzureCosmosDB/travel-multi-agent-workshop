...
✅ Deploy Azure Cosmos DB with all required containers (see list below)  
✅ Deploy Azure OpenAI with GPT-4 and text-embedding-3-small models  
✅ Configure managed identity and role assignments  
✅ Create a Python virtual environment (`.venv-travel`)  
✅ Install all Python dependencies from `requirements.txt` (includes the `azure-cosmos-agent-memory` toolkit and `langchain-azure-cosmosdb` checkpointer package)  
✅ Generate `python/.env` and `mcp_server/.env` with everything wired up, including the toolkit's cadence knobs (`FACT_EXTRACTION_EVERY_N`, `DEDUP_EVERY_N`, `THREAD_SUMMARY_EVERY_N`, `USER_SUMMARY_EVERY_N`) and the single canonical `COSMOSDB_DATABASE_NAME` database name  
✅ Seed Cosmos DB with initial data so the app is usable on first launch:
   ...
   - 14 pre-existing memories in `memories` and 28 conversational turns in `memories_turns` for Tony Stark and Steve Rogers

**Cosmos DB containers created (10 total):**

| Container             | Purpose                                                                                  |
|-----------------------|------------------------------------------------------------------------------------------|
| `Users`               | Application users (Tony, Steve, Peter, Bruce)                                            |
| `Places`              | Hotels, restaurants, activities (~2,900 docs with embeddings)                            |
| `Sessions`            | Chat sessions and message history                                                        |
| `Checkpoints`         | LangGraph short-term agent state (per-thread, async `CosmosDBSaver`)                     |
| `DebugLogs`           | Backend debug traces                                                                     |
| `Trips`               | Generated itineraries                                                                    |
| `memories`            | **Memory toolkit:** facts, episodic, and procedural memories (with vector embeddings)    |
| `memories_turns`      | **Memory toolkit:** raw user ↔ assistant turn buffer (no embedding, 30-day TTL)          |
| `memories_summaries`  | **Memory toolkit:** rolling thread and user summaries (created empty, filled by the SDK) |
| `counter`             | **Memory toolkit:** per-(user, thread) turn counters that drive the auto-trigger cadence |

> **Note on memory:** The `memories*` and `counter` containers are managed by the `azure-cosmos-agent-memory` toolkit you'll meet in Module 03. They are created up-front by the Bicep template so the toolkit can find them on first call.
...
Seeding users...
Seeding places...
Seeding memories (facts, episodic, procedural)...
Seeding conversational turns...
...
#### Memory Toolkit Containers

The `azure-cosmos-agent-memory` toolkit uses three containers (plus a `counter` helper). The Bicep template creates all four. Two are pre-seeded; the rest are populated naturally as users chat.

| Container             | Expected after `azd up`                  | Notes                                                                |
|-----------------------|------------------------------------------|----------------------------------------------------------------------|
| `memories`            | 14 documents (Tony 7, Steve 7)           | Facts, episodic, and procedural memories — each carries a 1536-dim embedding for hybrid recall |
| `memories_turns`      | 28 documents (Tony 14, Steve 14)         | Raw user/assistant turn records, no embedding, 30-day TTL            |
| `memories_summaries`  | 0 (empty)                                | Thread and user summaries — generated by the SDK as conversations grow |
| `counter`             | 0 (empty)                                | One row per active `(user_id, thread_id)` once chatting begins; drives the auto-trigger pipeline you'll tune in Module 04 |

Example **fact** document from `memories`:
...
```json
{
  ...
  "thread_id": "seed",
  "memory_type": "fact",
  "content": "Tony prefers luxury hotels with modern architecture",
  "salience": 0.95,
...
Example **turn** document from `memories_turns`:
...
```json
{
  ...
  "memory_type": "turn",
  "role": "user",
  "content": "Find me a luxury hotel in Barcelona near the beach.",
  "created_at": "2024-01-15T10:30:00Z"
}
```

We'll explore each container's role and the toolkit that writes to them, in Module 03.
...
`azd up` runs `python/data/seed_data.py` automatically. **You do not normally need to run it yourself.** Only fall back to this if the post-provision log clearly shows the seeding step failed (for example: an authentication error or a missing container while everything else provisioned cleanly).
...
The script is idempotent — re-running it is safe and will simply upsert the same documents.
