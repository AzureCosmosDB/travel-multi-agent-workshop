...
> **Where the filter runs — be precise.** The prune is only a *mark* on the memory doc; the exclusion happens in the **`recall_memories` MCP tool**, which filters `pruned` hits *after* the Cosmos memory toolkit's vector search returns them. So the saving is at the **agent's context** (stale memories aren't injected into the prompt → fewer tokens per turn) — *not* at the vector-search layer, which still scans the pruned docs. Pushing the filter into the memory toolkit's query itself would additionally cut retrieval cost.
